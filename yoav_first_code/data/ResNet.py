...
def get_dataloaders(data_dir: str, batch_size: int = 32, num_workers: int = 2):
    ...
    # Prints counts per class (very useful to spot imbalance)
...
def build_model(num_classes: int):
    # Use official pretrained weights (less error-prone than manual .pth)
    model = models.resnet18(weights=None)
    state_dict = torch.load("C:/Users/Your0124/pycharm_project_test/resnet18-f37072fd.pth", map_location="cpu")
...
def train_model(model, train_loader,valid_loader,device,
                phase1_epochs: int = 5,phase2_epochs: int = 10,):
    # Loss (option A): plain CE (sampler already handles imbalance well)
    ...
    # Optimizer: In Phase 1, we typically use a higher Learning Rate (e.g., 1e-3)
...
def main():
    ...
    model_path = os.path.join(output_dir, "id_classifier_resnet18_finetuned.pt")
