...
def main():
    ...
    while True:
        ...
        if results:
            ...
            face_to_save = smart_resize(face_crop, target_size=128)
        ...
        if face_to_save is not None:
            ...
            if key == ord('n'):
                ...
                path = os.path.join(FOLDERS['low_light'], f"real_dark_{timestamp}.jpg")
                cv2.imwrite(path, face_to_save)
                counts['low_light'] += 1
                print("Saved Dark Face")

            elif key == ord('b'):
                path = os.path.join(FOLDERS['motion_blur'], f"real_blur_{timestamp}.jpg")
                cv2.imwrite(path, face_to_save)
