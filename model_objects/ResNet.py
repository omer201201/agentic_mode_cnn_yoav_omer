...
# ----------------------------------------
# 1. Data Pipeline & Balancing
# PURPOSE: Loads images, applies augmentations, and ensures the model
# ----------------------------------------
# --- CONFIGURATION ---
...
def get_dataloaders(data_dir: str, batch_size: int = 32, num_workers: int = 2):
    ...
    # unmodified images. We only resize and normalize.
...
# PURPOSE: Loads a highly-educated "base" brain (ResNet18) and modifies it
# to specifically recognize the faces of your specific team members.
# ----------------------------------------
def build_model(num_classes: int):
    ...
    # final layer and attach a new one that only outputs three categories.
...
def set_trainable_params(model: nn.Module, phase: int):
    ...
    #----------------------------------------------
    #Transfer Learning Control Valve:
    #Phase 1: Freeze the body. Train ONLY the new head (Linear Probing).
    #Phase 2: Unfreeze the deepest layers (Layer 4) to let the model adapt to faces.
    #-----------------------------------------------
    # Phase 1: Freeze almost everything
    for p in model.parameters():
        p.requires_grad = False

    # # Always train the fully connected layer (the new head we just attached)
...
def get_optimizer(model: nn.Module, phase: int):
    ...
    # AdamW is an industry standard. 'weight_decay' prevents overfitting by
    # punishing the model if it becomes too "opinionated" about specific pixels.
    optimizer = optim.AdamW(params, lr=lr, weight_decay=1e-4)
    return optimizer


# -------------------------------------
# 3. Core Training & Evaluation Loops
# -------------------------------------
...
def train_model(model, train_loader, valid_loader, device, phase1_epochs: int = 5, phase2_epochs: int = 10, ):
    ...
    # If the validation loss stops dropping for 2 epochs,
...
def main():
    DATA_PATH = {
        "data_dir": os.path.join(PROJECT_ROOT, "data", "resnet_dataset"),
        "output_dir": os.path.join(PROJECT_ROOT, "models")
    }
    #data_dir = r"C:\Users\yoavt\PycharmProjects\final_projact\data\resnet_dataset"
    #output_dir = r"C:\Users\yoavt\PycharmProjects\final_projact\models"
    Path(DATA_PATH["output_dir"]).mkdir(parents=True, exist_ok=True)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print("Device:", device)

    # 1. Setup Data Pipeline
    train_loader, valid_loader, class_to_idx = get_dataloaders(DATA_PATH["data_dir"], batch_size=32, num_workers=2)
    ...
    save_path = MODEL_PATH["Resnet"]
    torch.save(model.state_dict(), save_path)
    print(f"Saved model weights to: {save_path}")

    mapping_path = os.path.join(DATA_PATH["output_dir"], "class_mapping.json")
