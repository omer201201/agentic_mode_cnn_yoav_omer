...
MODEL_PATH = {
    "BASE_TRAIN_DIR": os.path.join(PROJECT_ROOT, "data", "resnet dataset", "yoav_picture_224"),
    "MODEL_YOLO": os.path.join(PROJECT_ROOT, "models", "yolov8n-face.pt")
}


# Define the exact subfolders for each image degradation category
FOLDERS = {
    'normal': os.path.join(MODEL_PATH["BASE_TRAIN_DIR"], "normal"),
    'low_light': os.path.join(MODEL_PATH["BASE_TRAIN_DIR"], "low_light"),
    'motion_blur': os.path.join(MODEL_PATH["BASE_TRAIN_DIR"], "motion_blur"),
    'low_res': os.path.join(MODEL_PATH["BASE_TRAIN_DIR"], "low_res")
}
...
# A lightweight GUI tool that uses the webcam and YOLO to rapidly
# build a real-world dataset. The user physically acts out the degradations
# (e.g., turning off the lights) and presses keys to save the frames.
# ----------------------------------------
def main():
    print("Initializing Camera and YOLO...")
    detector = FaceDetector(model_path=MODEL_PATH["MODEL_YOLO"])

    # Open webcam (Change 0 to 1 if using external USB camera)
    ...
    # Track how many images we've captured in this session
    ...
    while True:
        ...
        # 1. Detect Face
        # expand_ratio=0.40 grabs a slightly wider box (40% larger) around the face
        # to ensure hair and chin are fully included in the training data.
        ...
        if results:
            ...
            # Show the live capture counts directly on the video feed
