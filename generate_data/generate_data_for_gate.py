...
# PURPOSE: Dynamically sets up the input/output directories for the data pipeline.
# ----------------------------------------
PROJECT_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
MODEL_PATH = {
    "SOURCE_DIR": os.path.join(PROJECT_ROOT, "data", "resnet dataset", "yoav_omer_befor_generate"),
    "ROOT_DIR": os.path.join(PROJECT_ROOT, "data", "resnet dataset","omer_yoav_after_generate_data")
}
...
def create_gate_folders():
    for split in ["train", "val"]:
        for c in ["normal", "low_light", "motion_blur", "low_res"]:
            path = os.path.join(MODEL_PATH["ROOT_DIR"], split, c)
            os.makedirs(path, exist_ok=True)


# ----------------------------------------
# 2. Image Resizing Utilities
# ----------------------------------------

def smart_resize(img, target_size=128):

    #Resizes image to target_size square using the optimal interpolation.
...
# ----------------------------------------
# 3. Image Degradation Generators (The Synthetic Engine)
# Artificially applies highly realistic digital and optical flaws
# to base images to train the Gate to recognize them.
# ----------------------------------------
def make_webcam_noise(img):
    ...
    noise = np.random.normal(1, 4, v.shape).astype(np.int16)
...
def make_low_light(img):
    ...
    # 3. Add Realistic Lens Softness
    if random.random() > 0.5:
        v_dark = cv2.GaussianBlur(v_dark, (5, 5), 0)
...
def make_motion_blur(img):
    ...
    kernel_size = random.choice([9, 11, 15, 17])
...
def generate():
    if not os.path.exists(MODEL_PATH["SOURCE_DIR"]):
        print(f"Error: Could not find {MODEL_PATH['SOURCE_DIR']}")
        return

    create_gate_folders()

    # Gather all images and randomize them
    all_images = list(Path(MODEL_PATH["SOURCE_DIR"]).glob("*.png")) + list(Path(MODEL_PATH["SOURCE_DIR"]).glob("*.jpg"))
    random.shuffle(all_images)

    # Slice the list to the maximum number requested
    ...
    print(f"\n Done! Real-time data generated at: {MODEL_PATH['ROOT_DIR']}")
